...
for (a = 2; a < process.argv.length; a++) {
  if (process.argv[a] == "-h" || process.argv[a] == "--help") {
    ...
    console.log(
      "SYNTH-DEFINITION is the encoded synth as found in the share URL at sfxr.me"
    );
    console.log(
      "You can also pipe the JSON Serialized version of the synth into the script like `cat my-sound.sfxr.json | ./sfxr-to-wav my-sound.wav`"
    );
    process.exit();
...
if (!parameters.set) {
  ...
  parameters.fromJSON(JSON.parse(fs.readFileSync("/dev/stdin").toString()));
}
